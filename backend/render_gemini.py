    ...
    for model_name in RENDER_MODELS:
        for attempt in range(retries + 1):
            try:
                ...
                last_exc = exc
                msg = str(exc)
                transient = ("500" in msg) or ("503" in msg) or ("UNAVAILABLE" in msg) or ("429" in msg)
                not_found = ("404" in msg) or ("not found" in msg.lower())
                ...
                if transient:
                    if attempt < retries:
                        ...
                        break # Move to the next model in RENDER_MODELS
                elif not_found:
                    break # Model doesn't exist, immediately try the next model
                else:
                    # Non-transient error (e.g. 400 Bad Request) that is not 404.
                    # It might be due to our prompt or image. We can try the next model just in case it handles it better.
                    break

    raise RuntimeError(f"Gemini render failed after trying all fallback models. Last error: {last_exc}")
